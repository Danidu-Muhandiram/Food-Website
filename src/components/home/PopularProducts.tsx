            ...
            <div className="container mx-auto px-4 md:px-8">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 md:mb-10">
                    <div className="space-y-4">
                        ...
                        <h2 className="text-3xl md:text-5xl lg:text-4xl xl:text-5xl font-black text-slate-900 tracking-tight leading-tight">
                            Our Most Popular <br className="hidden md:block" /> Products
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4 lg:gap-5">
                    {filteredProducts.map((product) => (
                        <div key={product.id} className="group bg-white rounded-[1.5rem] p-3 border border-slate-100 hover:border-[#E85C24]/30 hover:shadow-xl hover:shadow-orange-500/10 transition-all duration-500 flex flex-col">

                            {/* Image Container */}
                            <div className="relative aspect-square bg-[#F8F9FA] rounded-[1.2rem] mb-3 overflow-hidden">
                                {product.badge && (
                            ...
                            <div className="space-y-2 flex-1 flex flex-col">
                                <div className="flex justify-between items-start">
                                    <span className="text-slate-400 text-[10px] font-bold uppercase tracking-wide">{product.category}</span>
                                ...
                                <h3 className="font-bold text-slate-800 text-base leading-tight line-clamp-2 min-h-[2.5rem]">
                                    {product.name}
                                </h3>
                                ...
                                <div className="pt-2 mt-auto flex items-center justify-between">
                                    <div className="flex flex-col">
                                        <div className="flex items-baseline gap-2">
                                            <span className="text-xl font-black text-[#00A1E0]">${product.price.toFixed(2)}</span>
                                            {product.originalPrice && (
                                                <span className="text-xs font-bold text-slate-300 line-through">${product.originalPrice.toFixed(2)}</span>
                                            )}
                                        </div>
                                    </div>
                                    <button className="w-9 h-9 rounded-full bg-slate-50 text-slate-900 border border-slate-200 flex items-center justify-center hover:bg-[#00A1E0] hover:border-[#00A1E0] hover:text-white transition-all duration-300 shadow-sm">
